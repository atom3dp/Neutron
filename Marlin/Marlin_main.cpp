...
char selectedFilename[13] = "";
char previousFilename[13] = "";
...
//?�要添?��?�??应该?�在这个位置
   case 03:
       st_synchronize();//ptwu_1217
       digitalWrite(LASER_PIN,HIGH);//Pin?��??��??��??�制继电?��??�口（arduino Pin编号）。�??��??��?管�??��?，可?��?要�?置Pins.h?�件中相?�管?�为-1.
       break;
   case 04:
       analogWrite(LASER_PIN,8);//ptwu_對焦?��?�?4 來�??�弱?��?�?ptwu_1213
       break;
   case 05:
       st_synchronize();//ptwu_1217
...
//定制添�?�??结�?
    ...
    case 23: //M23 - Select file
      ...
      card.openFile(strchr_pointer + 4,true);
  	  strncpy(selectedFilename, strchr_pointer + 4, 13);
      break;
    case 24: //M24 - Start SD print
      card.startFileprint();
      starttime=millis();
	  strncpy(previousFilename, selectedFilename, 13);
      break;
...
void prepare_move()
{
  ...
  for (int s = 1; s <= steps; s++) {
    ...
    for(int8_t i=0; i < NUM_AXIS; i++) {
	  ...
	  if (destination[Z_AXIS] + z_offset + bed_level[3][3] > MANUAL_Z_HOME_POS)
	  {
		  destination[Z_AXIS] = MANUAL_Z_HOME_POS - bed_level[3][3] - z_offset ;
	  }
    calculate_delta(destination);
