...
static void lcd_main_menu()
{
	...
	MENU_ITEM(submenu, MSG_SETTING_ZOFFSET, lcd_setting_zoffset_menu);
#ifdef SDSUPPORT
    if (card.cardOK)
    {
        if (card.isFileOpen())
        {
			...
			if (fileCnt && previousFilename[0] != '\0')
			{
...
void load_filament()
{
    ...
    enquecommand_P((PSTR("G90")));//換回絕對最標
